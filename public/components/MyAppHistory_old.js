...
class MyAppHistory extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      days: 7,
      graphData: null,
      chartSeries: [{field: 'value', name: 'date'}]
    };
  }
  ...
  formatGraphData() {
    ...
    if (mode === 'total') {
      this.setState({chartSeries: [{field: 'value', name: 'date'}]});
      this.setState({graphData: data.Total});
    } else if (mode === 'server') {
      var results = {};
      var raw = this.props.state.myAppHistory.Servers[0].data;
      raw.forEach( item => {
        if ( (this.state.filterMode !== 'total' && this.state.filterOptions.indexOf(item.serverId) === -1) || !this.state.filterOptions.length) { return; }
      ...
      raw.forEach( item => {
        if ( (this.state.filterMode !== 'total' && this.state.filterOptions.indexOf(item.route) === -1) || !this.state.filterOptions.length ) { return; }
        if (!results.hasOwnProperty(item  .date)) {
          results[item.date] = {date: item.date};
        }
        var day = results[item.date];
  ...
  selectDay(value) {
    console.log(value)
    this.setState({days: value ? value.value : 7});
  }

  selectFilterMode(value) {
    console.log(value)
    this.setState({filterMode: value ? value.value : 'total'}, () => {
      this.setState({filterOptions: []}, () => this.formatGraphData());
    });
  }

  selectFilterOptions(values) {
    console.log(values)
    this.setState({filterOptions: values.length ? _.pluck(values, 'value') : []}, () => this.formatGraphData());
  }
